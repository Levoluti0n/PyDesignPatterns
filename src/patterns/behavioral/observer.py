    ...
    def update(self, subject: Subject):
        if isinstance(subject, Stock):
            if subject.get_price() > self.threshold:
                print(f"Trader {self.name}: Stock {subject.get_symbol()} price has crossed the threshold! Price: {subject.get_price()}")
...
class StockPriceAlert(Observer):
    ...
    def update(self, subject: Subject):
        if isinstance(subject, Stock):
            print(f"Alert: {self.alert_message} - {subject.get_symbol()} New Price: {subject.get_price()}")
...
class StockInvestmentTracker(Observer):
    ...
    def update(self, subject: Subject):
        if isinstance(subject, Stock):
            ...
            print(f"Investment Tracker: New total investment value in {subject.get_symbol()} is: {investment_value}")
